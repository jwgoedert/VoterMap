...
function loadData(error, usData, AllNewCountyData, AllDropCountyData) {
  ...
  d3.select(".state-header")
    .text(stateName);
  // set projection with Alaska outlier check
  ...
  // creating color determining function which takes a view as an argument
  setColor = function (v) {
  ...
  projection
    .scale(1)
    .translate([0, 0])
  // set projection boundaries based on state ID
  ...
  function click(d) {
    ...
    d3.select(this)
      .style("fill", "orange");
    d3.select(".selected")
      .text(`Selected: ${ countyByView(d).County } ${ countyByView(d).key_pct } % `);
  }
